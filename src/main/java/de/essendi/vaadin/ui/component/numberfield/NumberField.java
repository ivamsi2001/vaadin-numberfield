...
@com.vaadin.ui.ClientWidget(VNumberField.class)
public class NumberField extends TextField {
	...
	private boolean validateValue(Object value) {
		...
		if (isDecimalAllowed()) {
			isValid = NumberValidator
					.isValidDecimal((String) value, attributes);
		} else {
			isValid = NumberValidator
					.isValidInteger((String) value, attributes);
		}
	...
	public void paintContent(PaintTarget target) throws PaintException {
		...
		target.addAttribute(ATTRIBUTE_ALLOW_DECIMALS, isDecimalAllowed());
		target.addAttribute(ATTRIBUTE_ALLOW_NEGATIVES, isNegativeAllowed());
		target.addAttribute(ATTRIBUTE_DECIMAL_PRECISION, getDecimalPrecision());
		target.addAttribute(ATTRIBUTE_DECIMAL_SEPARATOR,
				(int) getDecimalSeparator());
		target.addAttribute(ATTRIBUTE_USE_GROUPING, isGroupingUsed());
		target.addAttribute(ATTRIBUTE_GROUPING_SEPARATOR,
	...
	public void setValue(Object newValue) throws IllegalArgumentException,
		...
		if (!(newValue instanceof String)) {
			throw new IllegalArgumentException(
					"NumberField#setValue() expects a String as argument.");
		}
